...
static double msecToTimecents(double ms) {
	const float cents = 1200.0f * (float)log2(ms / 1000.0);
	if(cents < -32768.0f)
		return -32768.0;
	else
		return cents;
...
void ss_channel_nrpn_awe32(SS_MIDIChannel *ch, int awe_gen, int data_lsb, int data_msb, double time) {
	...
	switch(generator) {
		...
		case SS_GEN_DELAY_MOD_ENV:
			milliseconds = 4 * clip((float)data_value, 0, 5900);
			// Convert to timecents
			ss_channel_set_generator_override(ch, generator, (int)msecToTimecents(milliseconds), false, time);
			break;

		/* Attacks */
		case SS_GEN_ATTACK_VOL_ENV:
		case SS_GEN_ATTACK_MOD_ENV:
			milliseconds = clip((float)data_value, 0, 5940);
			/* Convert to timecents */
			ss_channel_set_generator_override(ch, generator, (int)msecToTimecents(milliseconds), false, time);
			break;
		...
		case SS_GEN_HOLD_MOD_ENV:
			milliseconds = clip((float)data_value, 0, 8191);
			/* Convert to timecents */
			ss_channel_set_generator_override(ch, generator, (int)msecToTimecents(milliseconds), false, time);
			break;
		...
		case SS_GEN_RELEASE_MOD_ENV:
			milliseconds = 4.0f * clip((float)data_value, 0, 5940);
			/* Convert to timecents */
			ss_channel_set_generator_override(ch, generator, (int)msecToTimecents(milliseconds), false, time);
			break;
		...
		case SS_GEN_FREQ_MOD_LFO:
			hertz = 0.084f * (float)data_lsb;
			/* Convert to abs cents */
			ss_channel_set_generator_override(ch, generator, (int)hzToCents(hertz), true, time);
			break;
		...
		case SS_GEN_SUSTAIN_MOD_ENV:
			/* 0.75 dB is 7.5 cB */
			centibels = (float)data_lsb * 7.5f;
			ss_channel_set_generator_override(ch, generator, (int)centibels, false, time);
			break;
		...
		case SS_GEN_VIB_LFO_TO_PITCH:
			cents = clip((float)data_value, -127.0f, 127.0f) * 9.375f;
			ss_channel_set_generator_override(ch, generator, (int)cents, true, time);
			break;

		/* Env to pitch */
		case SS_GEN_MOD_ENV_TO_PITCH:
			cents = clip((float)data_value, -127.0f, 127.0f) * 9.375f;
			ss_channel_set_generator_override(ch, generator, (int)cents, false, time);
			break;

		/* Mod LFO to vol */
		case SS_GEN_MOD_LFO_TO_VOLUME:
			/* 0.1875 dB is 1.875 cB */
			centibels = 1.875f * (float)data_lsb;
			ss_channel_set_generator_override(ch, generator, (int)centibels, true, time);
			break;

		/* Filter Fc */
		case SS_GEN_INITIAL_FILTER_FC: {
			/* Minimum: 100 Hz -> 4335 cents */
			const float fc_cents = 4335.0f + 59.0f * (float)data_lsb;
			ss_channel_set_generator_override(ch, generator, (int)fc_cents, true, time);
			break;
		}
		...
		case SS_GEN_INITIAL_FILTER_Q:
			 ...
			 */
			centibels = 215.0f * ((float)data_lsb / 127.0f);
			ss_channel_set_generator_override(ch, generator, (int)centibels, true, time);
			break;

		/* To filter Fc */
		case SS_GEN_MOD_LFO_TO_FILTER_FC:
			cents = clip((float)data_value, -64.0f, 63.0f) * 56.25f;
			ss_channel_set_generator_override(ch, generator, (int)cents, true, time);
			break;
		...
		case SS_GEN_REVERB_EFFECTS_SEND:
			ss_channel_set_generator_override(ch, generator, (int)(clip((float)data_value, 0, 255.0f) * (1000.0f / 255.0f)), false, time);
			break;
	}
}
