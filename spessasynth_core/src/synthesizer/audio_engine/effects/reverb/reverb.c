...
static void ss_reverb_update_feedback(SS_Reverb *reverb) {
	...
	const double x = (double)p->delayFeedback / 127.0;
	const double exp = 1.0 - pow(1.0 - x, 1.9);
	if(p->character == 6) {
		reverb->delayLeft->feedback = exp * 0.73;
	} else {
		reverb->delayLeft->feedback = reverb->delayRight->feedback = 0;
		reverb->panDelayFeedback = exp * 0.73;
	}
}

static void ss_reverb_update_lowpass(SS_Reverb *reverb) {
	const SS_ReverbParams *p = &reverb->parameters;
	const double preLPF = 0.1 + (double)(7 - p->preLowpass) / 14.0 + reverb->characterLPFCoefficient;
	reverb->dattorro->preLPF = (preLPF < 1.0) ? preLPF : 1.0;
}

static void ss_reverb_update_gain(SS_Reverb *reverb) {
	const SS_ReverbParams *p = &reverb->parameters;
	reverb->dattorro->gain = ((float)p->level / 345.0) * reverb->characterGainCoefficient;
	reverb->delayGain = ((float)p->level / 127.0) * 1.5;
}

static void ss_reverb_update_time(SS_Reverb *reverb) {
	const SS_ReverbParams *p = &reverb->parameters;
	const double t = (double)p->time / 127.0;
...
void ss_reverb_set_character(SS_Reverb *reverb, unsigned char character) {
	...
	switch(character) {
		case 0: {
			...
			reverb->characterGainCoefficient = 0.9;
		...
		case 3: {
			// Hall1
			dattorro->damping = 0.3;
			reverb->characterGainCoefficient = 1.25;
			reverb->characterTimeCoefficient = 1.3;
			reverb->characterLPFCoefficient = 0;
		...
		case 4: {
			// Hall2
			reverb->characterGainCoefficient = 1;
			reverb->characterTimeCoefficient = 1.2;
			reverb->characterLPFCoefficient = 0.1;
			dattorro->damping = 0.1;
			dattorro->decayDiffusion[1] = 0.69;
			dattorro->decayDiffusion[0] = 0.67;
			break;
		}
...
void ss_reverb_set_pre_delay_time(SS_Reverb *reverb, unsigned char preDelayTime) {
	...
	reverb->dattorro->preDelay = (unsigned int)(((float)preDelayTime / 1000.0) * reverb->sampleRate);
}
...
void ss_reverb_set_pre_lowpass(SS_Reverb *reverb, unsigned char preLowpass) {
	...
	reverb->preLPFfc = 8000.0 * pow(0.63, (float)preLowpass);
	const double decay = exp((-2.0 * M_PI * reverb->preLPFfc) / reverb->sampleRate);
	...
	switch(reverb->parameters.character) {
		...
		case 6: {
			...
			if(reverb->parameters.preLowpass > 0) {
				...
				double z = reverb->preLPFz;
				const double a = reverb->preLPFa;
