...
extern double ss_convex_attack(int index_0_to_999);

double ss_modulation_envelope_get_value(const SS_ModulationEnvelope *env,
                                        double current_time);
...
static double tc2Sec(double timecents) {
	 ...
	 */
	if(timecents <= -10114.0) return 0.0;
...
void ss_modulation_envelope_recalculate(SS_ModulationEnvelope *env,
	...
	double decay_time = tc2Sec((double)(mod_gens[SS_GEN_DECAY_MOD_ENV] + decay_key_exc));
	env->decay_duration = decay_time * (1.0f - env->sustain_level);

	int hold_key_exc = (60 - midi_note) * mod_gens[SS_GEN_KEYNUM_TO_MOD_ENV_HOLD];
	env->hold_duration = tc2Sec((double)(hold_key_exc + mod_gens[SS_GEN_HOLD_MOD_ENV]));

	int rel_tc = mod_gens[SS_GEN_RELEASE_MOD_ENV];
	if(rel_tc < -7200) rel_tc = -7200;
	const double rel_time = tc2Sec((double)rel_tc);
...
void ss_modulation_envelope_start_release(SS_ModulationEnvelope *env,
	...
	double releasecents = mod_gens[SS_GEN_RELEASE_MOD_ENV];
	if(releasecents < -7200.0) releasecents = -7200.0;
	const double release_time = tc2Sec(releasecents);
...
double ss_modulation_envelope_get_value(const SS_ModulationEnvelope *env,
	...
	if(e->entered_release) {
		...
		double value = ((1.0 - (current_time - e->release_start_time) / e->release_duration) * e->release_start_level);
